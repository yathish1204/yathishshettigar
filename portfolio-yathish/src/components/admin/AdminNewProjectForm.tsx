...
export default function AdminNewProjectForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    responsibilities:
      "Frontend Architecture, Accessibility Testing, UX Engineering",
    challenge: "",
    research: "",
    designProcess: "",
    solution: "",
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between border-b border-zinc-800 pb-4">
        <div>
          <Link
            href="/admin/projects"
            className="text-xs text-zinc-400 hover:text-[#B45309] dark:hover:text-[#FBBF24] font-mono"
          >
      ...
      {error && (
        <div
          role="alert"
          className="p-4 rounded-xl bg-red-950/60 border border-red-800 text-red-300 text-xs"
        >
          {error}
        </div>
      )}
      ...
      <form className="space-y-6 bg-zinc-900/60 p-6 md:p-8 rounded-2xl border border-zinc-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-mono uppercase tracking-wider text-zinc-300 mb-1">
              {formData.isCorporateProject
                ? "Thumbnail URL (Optional for Corporate)"
                : "Thumbnail URL *"}
            </label>
            <input
              type="text"
