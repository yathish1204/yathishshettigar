...
export function Footer({ profile }: { profile: Profile }) {
  const phoneTel = profile.phone ? profile.phone.replace(/\D/g, '') : '919900000000';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          ...
          <div className="md:col-span-2 space-y-4">
            <Link href="/" className="text-xl font-bold text-zinc-900 dark:text-zinc-100 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors">
              {profile.name}
            </Link>
          ...
          <div className="space-y-3">
            ...
            {/* Grid layout strictly limited to 4 columns (4 icons per row) */}
              ...
              {socialItems.map((item) => (
                <div key={item.name} className="relative group">
                  <a
                    ...
                    className="w-10 h-10 rounded-xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300 flex items-center justify-center hover:bg-emerald-500 hover:text-zinc-950 dark:hover:bg-emerald-500 dark:hover:text-zinc-950 hover:border-emerald-500 transition-all shadow-sm hover:scale-110 cursor-pointer"
                  >
                    {item.icon}
                  </a>
                  {/* Floating Hover Tooltip */}
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-zinc-900 dark:bg-zinc-100 text-zinc-100 dark:text-zinc-900 text-[10px] font-mono font-bold rounded shadow-md opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-20">
                    {item.name}
