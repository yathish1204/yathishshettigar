...
export function Footer({ profile }: { profile: Profile }) {
  ...
  const socialItems = [
    ...
    {
      name: 'Facebook',
      href: profile.socialLinks?.facebook || 'https://facebook.com/yathish',
      icon: (
        <svg className="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-12">
          ...
          <div className="md:col-span-2 space-y-4">
            <Link href="/" className="text-xl font-bold text-zinc-900 dark:text-zinc-100 hover:text-[#B45309] dark:hover:text-[#FBBF24] transition-colors">
              {profile.name}
            </Link>
            <p className="text-sm text-zinc-600 dark:text-zinc-400 md:mt-2 max-w-md leading-relaxed">{profile.tagline}</p>
            
          </div>

          {/* Quick Links */}
          <nav className="space-y-3" aria-label="Footer navigation">
            <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-800 dark:text-zinc-200 font-mono">Navigation</h4>
            <ul className="space-y-2">
              <li>
                <Link href="/projects" className="hover:text-[#B45309] dark:hover:text-[#FBBF24] transition-colors">
                  Projects / Case Studies
                </Link>
              </li>
              <li>
                <Link href="/about" className="hover:text-[#B45309] dark:hover:text-[#FBBF24] transition-colors">
                  About & Methodology
                </Link>
              </li>
              <li>
                <Link href="/experience" className="hover:text-[#B45309] dark:hover:text-[#FBBF24] transition-colors">
                  Experience & History
                </Link>
              </li>
              <li>
                <Link href="/contact" className="hover:text-[#B45309] dark:hover:text-[#FBBF24] transition-colors">
                  Get in Touch
                </Link>
              </li>
              <li>
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-zinc-800 dark:text-zinc-200 font-mono">Connect</h4>
            
            {/* Flex layout with comfortable breathing gap between icons */}
            <div className="flex flex-wrap items-center gap-3.5 sm:gap-4" role="list" aria-label="Social media links">
              {socialItems.map((item) => (
                <div key={item.name} className="relative group" role="listitem">
                  ...
                  <div aria-hidden="true" className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-zinc-900 dark:bg-zinc-100 text-zinc-100 dark:text-zinc-900 text-[10px] font-mono font-bold rounded shadow-md opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity whitespace-nowrap z-20">
                    {item.name}
